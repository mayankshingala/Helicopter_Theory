...
for i = 1:1:301
    ...
    lambda(i+1) = mu(i)*tan(alpha) + Ct/(2*sqrt(mu(i)^2 + lambda(i)^2));
    lambda_fpi(i) = lambda(i+1);
end

% Plotting of data (Fixed Point Iteration)
plot(mu,lambda_fpi,'r')
hold on
plot(mu,lambda_apprx,'--k')
xlim([0 0.6])
ylim([0 0.08])
title('µ v/s λ (Fixed Point Iteration & Approximate Solution)')
xlabel('Advance Ratio, µ')
ylabel('Inflow Ratio, λ')
legend('Fixed Point Iteration','Approximate Solution')
grid on
hold off
